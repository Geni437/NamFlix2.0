...
export default function ChannelPage() {
  ...
  useEffect(() => {
    ...
    // Log watch history (fire-and-forget)
    apiFetch('/me/history', {
      method: 'POST',
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 space-y-5">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1.5">
                  {channel.country && (
                    <Link
                      href={`/country/${channel.country}/`}
                      className="text-text-secondary text-sm hover:text-accent-blue transition-colors uppercase tracking-wide"
                    >
                      {channel.country}
                    </Link>
                  )}
                  {categories.slice(0, 3).map(cat => (
                    <Link
                      key={cat}
                      href={`/category/${cat}/`}
                      className="px-2 py-0.5 bg-surface-2 border border-border rounded text-xs text-text-secondary hover:text-white hover:border-border transition-colors"
                    >
                      {cat}
                    </Link>
                  ))}
            ...
            <div className="flex items-center gap-3 flex-wrap">
              {streams[0] && (
                <button
                  onClick={() => { setReportStreamId(streams[0].id); setReportModal(true); }}
